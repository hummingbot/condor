...
# Solana inventory wallets (both tracked since the 2026-07-07 Swig cutover):
# the Swig funds-owner holds the narrow position + trading float; the legacy
# botcamp wallet holds the wide position + a SOL reserve.
SOLANA_WALLETS = {
    "29hB4hN13nm79A2BrWxVzWo3KNrjxyNKrXgetBWe6xzh": "Swig funds-owner (narrow + float)",
    "DQcmxgGCEwThGCzV6NmFG2WsbUpch3HLoZAhctcgeRM9": "Botcamp legacy (wide + SOL reserve)",
}
        ...
        for amt in amounts:
            ...
            if b_ok and s_ok:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name="unsupported", showlegend=(row == 1 and venue == pool_venues[0]),
                    legendgroup="na",
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        # ── Solana wallet inventory (both wallets since the Swig cutover) ─
        if wallet_data:
            ...
            builder.markdown(
                "## Solana wallets & Orca positions\n"
                f"Total tracked inventory ≈ **${total_usd:,.2f}** "
                "(USDC + USDM1 at Solana NAV, wallet float + position holdings; SOL excluded)."
            )

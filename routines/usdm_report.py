...
async def _fetch_wallet(session, base: str, address: str) -> dict:
    ...
    try:
        # /trading/clmm/positions-owned takes no poolAddress (the schema
        # rejects unknown properties), so it returns every Orca position the
        # wallet owns and we narrow to the USDM1/USDC pool here.
        all_positions = await _api(
            session,
            base,
            "/trading/clmm/positions-owned",
            {
                "connector": "orca",
                "chainNetwork": CHAINS["solana"]["chain_network"],
        ...
        out["positions"] = [
            ...
            if p.get("poolAddress") == ORCA_POOL
        ]
    except Exception as e:
...
def _coerce_position(pos: dict) -> dict:
    """/trading/clmm/positions-owned returns amounts and prices as strings;
    the report does arithmetic on them, so convert once at ingestion."""
    c = dict(pos)
    for k in (
        "baseTokenAmount",
        "quoteTokenAmount",
...
def _coerce_bins(bins: list) -> list:
    """/trading/clmm/pool-info returns bin amounts and prices as strings; every
    consumer here treats them as numbers, so convert once at ingestion."""
    out = []
    for b in bins:
        c = dict(b)
        for k in ("price", "baseTokenAmount", "quoteTokenAmount"):
            if c.get(k) is not None:
                c[k] = float(c[k])
        out.append(c)
    return out
...
async def _fetch_pool_info(
    session, base: str, dex: str, chain_network: str, pool: str, bin_count: int = 0
) -> Optional[dict]:
    try:
        params: dict = {
            "connector": dex,
        ...
        r = await _api(session, base, "/trading/clmm/pool-info", params)
        info = {
            ...
            "bins": _coerce_bins(r.get("bins") or []),
        }
...
async def _spot_from_quote(
    session, base: str, dex: str, chain_network: str, pool: str, slippage: float
) -> Optional[float]:
    q = await _quote_pool(session, base, dex, chain_network, pool, "BUY", 0.01, slippage)
...
async def _probe(
    ...
    chain_network = CHAINS[chain]["chain_network"]
    q = await _quote_pool(
        session, base, meta["dex"], chain_network, pool, side, amount, cfg.slippage_pct
    )
    if not q:
        ...
        for v in pool_venues:
            ...
            pool_info_tasks.append(
                _fetch_pool_info(
                    ...
                    base,
                    meta["dex"],
                    CHAINS[meta["chain"]]["chain_network"],
                    pool,

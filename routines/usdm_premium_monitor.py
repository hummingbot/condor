...
FLEET = {
    "0x138c8b01fa4f1771730a211df6613e349379d52e",
...
class Config(BaseModel):
    ...
    threshold_bps: float = Field(
        default=50.0, description="Alert when pool premium over NAV ≥ this many bps"
    )
    whale_trade_usd: float = Field(
        default=5000.0,
        description="Alert immediately on any single non-us swap ≥ this USD size",
    )
    probe_usdc: float = Field(
        default=1500.0,
        description="Sell-probe size in USDC used to project the per-cycle arb edge",
    )
    interval_sec: int = Field(default=300, description="Check interval in seconds")
    cooldown_min: int = Field(
        default=60, description="Minutes between repeat premium alerts"
    )
...
async def _rpc(session: aiohttp.ClientSession, payload) -> dict:
    async with session.post(
        ETH_RPC, json=payload, timeout=aiohttp.ClientTimeout(total=30)
    ) as r:
        return await r.json()
...
async def _api(
    ...
    async with session.get(
        f"{GATEWAY_URL}{path}", params=params, timeout=aiohttp.ClientTimeout(total=30)
    ) as r:
        text = await r.text()
        if not r.ok:
...
async def _fetch_new_swaps(
    ...
    if "error" in r:
        raise RuntimeError(
            f"eth_getLogs: {r['error'].get('message', r['error'])[:120]}"
        )
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    chat_id = (
        context._chat_id
        if context is not None and hasattr(context, "_chat_id")
        else None
    )
    ...
    async def render_report(c: dict) -> None:
        ...
        if c["edge_usd"] is not None:
            report.builder.kpi(
                f"Edge / ${config.probe_usdc:,.0f} cycle", f"{c['edge_usd']:+,.2f} USDC"
            )
        ...
        if len(prem_hist) >= 2:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name="premium",
                    hovertemplate="%{x|%b %d %H:%M}<br>%{y:+.1f} bps<extra></extra>",
                )
            )
            fig.add_hline(
                y=config.threshold_bps,
                line=dict(color="#ef4444", width=1, dash="dash"),
                annotation_text=f"alert {config.threshold_bps:.0f} bps",
            )
            fig.add_hline(
                y=0, line=dict(color="rgba(255,255,255,0.35)", width=1, dash="dot")
            )
            for s in trade_hist:
                if s["actor"] != "us" and s["usd"] >= config.whale_trade_usd:
                    fig.add_vline(
                        x=_dt.datetime.fromtimestamp(s["ts"]),
            ...
            fig.update_layout(
                ...
                title_text="Pool premium over NAV (bps)",
                title_font_size=14,
                showlegend=False,
            )
        ...
        if trade_hist:
            ...
            for s in trade_hist[-25:][::-1]:
                rows.append(
                    {
                        "Time (UTC)": _dt.datetime.utcfromtimestamp(s["ts"]).strftime(
                            "%m-%d %H:%M"
                        ),
                        "Actor": s["actor"],
                        "Side": s["side"],
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            return (
                f"usdm_premium_monitor: startup check failed — Gateway at {GATEWAY_URL} "
        ...
        while True:
            try:
                ...
                swaps = (
                    await _fetch_new_swaps(session, last_block, block)
                    if block > last_block
                    else []
                )
                last_block = block
                ...
                rec = {
                    "ts": int(now),
                    "premium_bps": round(c["premium_bps"], 2),
                    "spot": c["spot"],
                    "nav": c["nav"],
                }
                if swaps:
                ...
                try:
                    ...
                    logger.error(
                        f"usdm_premium_monitor: report render failed: {str(e)[:150]}"
                    )
                ...
                for s in swaps:
                    if s["actor"] != "us" and s["usd"] >= config.whale_trade_usd:
                        who = {
                            "whale": "the Aug-4 whale 0xca7315a9…",
                            "fleet": "the bot fleet",
                        }.get(s["actor"], f"NEW wallet {s['from'][:10]}…")
                        await notify(
                ...
                if c["premium_bps"] >= config.threshold_bps:
                    ...
                    await notify(
                        f"✅ USDM1 ETH premium back under {config.threshold_bps:.0f} bps "
                ...
                if swaps:
                    logger.info(
                        f"usdm_premium_monitor: {len(swaps)} new swap(s): "
                        + "; ".join(
                ...
                errors += 1
                logger.error(
                    f"usdm_premium_monitor: check failed ({errors}): {str(e)[:150]}"
                )
                if errors >= 5:
                    await notify(
                        f"🔴 USDM1 monitor stopping after {errors} consecutive failures "

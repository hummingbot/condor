...
def _calc_base_pct(
    pos: dict, token_prices: dict, base_symbol: str, quote_symbol: str
) -> float:
...
async def _notify_back_in_range(
    context, chat_id: int, pos: dict, token_cache: dict
) -> None:
...
async def _notify_rebalance(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🔄 Rebalance", callback_data=f"dex:lpm_rebal:{cache_key}"
            ),
            InlineKeyboardButton(
                "✅ Dismiss", callback_data=f"dex:lpm_dismiss:{instance_id}"
            ),
...
async def _countdown_action(
    ...
    for remaining in [30, 20, 10, 5]:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "❌ Cancel", callback_data=f"dex:lpm_cancel:{cache_key}"
                ),
            ]
        ]
...
async def _execute_close(
    context, chat_id: int, msg_id: int, pos: dict, client, token_cache: dict
) -> None:
...
async def _execute_rebalance(
    ...
    try:
        ...
        base_amt = float(
            close_result.get("base_amount", close_result.get("amount_base", 0)) or 0
        )
        quote_amt = float(
            close_result.get("quote_amount", close_result.get("amount_quote", 0)) or 0
        )
        ...
        if not quote_amt:
            quote_amt = float(
                pos.get("quote_token_amount", pos.get("amount_b", 0)) or 0
            )
        ...
        if open_result:
            tx = (open_result.get("tx_hash") or open_result.get("signature", "N/A"))[
                :16
            ]
            text = (
...
async def _send_status_report(
    ...
    for pos in positions:
        ...
        total_value += (
            float(pnl_summary.get("current_lp_value_quote", 0) or 0) * quote_price
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                "💰 Collect All Fees",
                callback_data=f"dex:lpm_collect_all:{instance_id}",
            )
        ]
    )
    keyboard.append(
        [
            InlineKeyboardButton(
                "⏹ Stop Monitor", callback_data=f"routines:stop:{instance_id}"
            )
        ]
    )
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    logger.info(
        f"LP Monitor starting: interval={config.check_interval_sec}s, report={config.status_report_hours}h"
    )
        ...
        while True:
            try:
                ...
                positions = [
                    ...
                    and float(p.get("liquidity", p.get("current_liquidity", 1)) or 1)
                    > 0
                ]
                ...
                networks = set(
                    p.get("network", "solana-mainnet-beta") for p in positions
                )
                ...
                for pos in positions:
                    ...
                    if (
                        in_range == "IN_RANGE"
                        and pos_id not in state["rebalance_notified"]
                    ):
                        base_sym = resolve_token_symbol(
                            pos.get("base_token", ""), token_cache
                        )
                        quote_sym = resolve_token_symbol(
                            pos.get("quote_token", ""), token_cache
                        )
                        base_pct = _calc_base_pct(
                            pos, token_prices, base_sym, quote_sym
                        )
                ...
                if state["checks"] % 10 == 0:
                    logger.info(
                        f"LP Monitor #{state['checks']}: {len(positions)} positions, {state['alerts']} alerts"
                    )

...
async def _get_order_book(
    client, connector: str, trading_pair: str, depth: int
) -> dict | None:
    try:
...
async def _get_fill_price(
    client, connector: str, trading_pair: str, amount: float, is_buy: bool
) -> float | None:
    try:
...
def _parse_level(level) -> tuple[float, float]:
    if isinstance(level, dict):
        ...
        size = float(
            level.get(
                "quantity",
                level.get("size", level.get("amount", level.get("Quantity", 0))),
            )
        )
        return price, size
...
def _ob_stats(bids: list, asks: list) -> dict:
    ...
    spread_pct = (
        ((best_ask - best_bid) / best_bid * 100) if best_bid and best_ask else None
    )
...
def _compute_arb_routes(
    valid_exchanges: list[str], ex_data: dict, amount: float
) -> list[dict]:
    ...
    for ex_a, ex_b in combinations(valid_exchanges, 2):
        ...
        if da["fill_buy"] and db["fill_sell"]:
            ...
            routes.append(
                {
                    "route": f"BUY {ex_a.upper()} → SELL {ex_b.upper()}",
                    "spread": s,
                    "profit": profit,
                }
            )
        if db["fill_buy"] and da["fill_sell"]:
            ...
            routes.append(
                {
                    "route": f"BUY {ex_b.upper()} → SELL {ex_a.upper()}",
                    "spread": s,
                    "profit": profit,
                }
            )
...
def _filter_and_sort_exchanges(
    exchanges: list[str], ex_data: dict
    ...
    if len(candidates) >= 2:
        ...
        for ex in candidates:
            ...
            if median_mid > 0 and abs(mid - median_mid) / median_mid > 0.5:
                invalid.append(
                    (ex, f"Price outlier (mid={mid:.8g} vs median={median_mid:.6f})")
                )
            else:
    ...
    candidates.sort(
        key=lambda ex: ex_data[ex]["stats"].get("spread_pct") or float("inf")
    )
        ...
        if bids:
            fig.add_trace(
                go.Scatter(
                    ...
                    name="Bids",
                    hovertemplate="Price: %{x:.6f}<br>Cumul: %{y:.2f}<extra></extra>",
                    showlegend=(row == 1),
                    legendgroup="bids",
                ),
                row=row,
        ...
        if bids and asks and ref_mid and ref_mid > 0:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers+text",
                    marker=dict(
                        color=c_bid, size=13, line=dict(width=1.5, color="#1a1a2e")
                    ),
                    text=[f"{bid_bps:+.1f}"],
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers+text",
                    marker=dict(
                        color=c_ask, size=13, line=dict(width=1.5, color="#1a1a2e")
                    ),
                    text=[f"{ask_bps:+.1f}"],
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if len(exchanges) < 2:
        return (
            "Need at least 2 exchanges (comma-separated). Example: binance,kucoin,htx"
        )
    ...
    buy_tasks = [
        _get_fill_price(client, ex, pair, config.amount, True) for ex in exchanges
    ]
    sell_tasks = [
        _get_fill_price(client, ex, pair, config.amount, False) for ex in exchanges
    ]
    ...
    for i, ex in enumerate(exchanges):
        ...
        ex_data[ex] = {
            ...
            "asks": asks,
            "stats": stats,
            "fill_buy": buy_results[i],
            "fill_sell": sell_results[i],
        }
    ...
    if not valid_exchanges:
        ...
        if invalid_exchanges:
            msg += "\n\nFiltered out:\n" + "\n".join(
                f"  {ex.upper()}: {reason}" for ex, reason in invalid_exchanges
            )
        return msg
    ...
    for ex in valid_exchanges:
        ...
        if d["stats"]:
            s = d["stats"]
            lines.append(
                f"  Best Bid: {s['best_bid']:.6f} | Best Ask: {s['best_ask']:.6f}"
            )
            lines.append(f"  Mid: {s['mid']:.6f} | Spread: {s['spread_pct']:.4f}%")
            lines.append(
                f"  Depth: Bid ${s['bid_depth_usd']:,.0f} | Ask ${s['ask_depth_usd']:,.0f} ({s['n_bid_levels']}/{s['n_ask_levels']} levels)"
            )
        if d["fill_buy"]:
    ...
    builder.source("routine", "arb_check").tags(
        ["arbitrage", "cex-cex", pair] + valid_exchanges
    )
    ...
    best_arb = (
        max(all_arb_routes, key=lambda r: r["spread"]) if all_arb_routes else None
    )

    if best_arb:
        builder.kpi(
            "Best Route",
            f"{best_arb['route']}  {best_arb['spread']:+.4f}%",
    ...
    for ex in valid_exchanges:
        ...
        row[f"Fill BUY {config.amount}"] = (
            f"{d['fill_buy']:.6f}" if d["fill_buy"] else "N/A"
        )
        row[f"Fill SELL {config.amount}"] = (
            f"{d['fill_sell']:.6f}" if d["fill_sell"] else "N/A"
        )
    ...
    if invalid_exchanges:
        ...
        filtered_rows = [
            {"Exchange": ex.upper(), "Reason": reason}
            for ex, reason in invalid_exchanges
        ]

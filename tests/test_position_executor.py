...
def config(**over):
    kw = dict(
        ...
        take_profit_pct=Decimal("0.10"), stop_loss_pct=Decimal("0.05"),
        time_limit_s=3600, update_interval=0.01,
    )
    kw.update(over)
    return PositionConfig(**kw)
...
def test_time_limit_closes_even_without_price(tmp_path):
    ...
    rec, _ = run_executor(
        tmp_path, NoPriceAfterOpen([100.0]), config(time_limit_s=0))
    assert rec.status == "CLOSED"
...
def test_trailing_stop_ratchets_and_closes(tmp_path):
    ...
    cfg = config(take_profit_pct=None, stop_loss_pct=None,
...
def test_reconcile_readopts_active_position(tmp_path):
    ...
    ex.state.base_bought = Decimal("0.0002")
    ...
    # resumed monitoring hits the 111 quote -> take profit
    assert rec.status == "CLOSED"
    assert rec.state["close_type"] == "take_profit"


def test_reconcile_fails_loud_mid_opening(tmp_path):
    gateway = FakeGateway([100.0])
    store = ExecutorStore(tmp_path / "t.db")

...
def test_surfpool_answer_carries_host_only(configured, monkeypatch):
    async def fake_probe(url):
        assert url == "http://127.0.0.1:8899"
        return {"kind": "surfpool", "rpc_host": "127.0.0.1:8899", "surfnet_version": "1.5.0",
                "solana_core": "4.1.2", "slot": 42}
    ...
    assert "rpcUrl" not in r.text and "http://" not in body["rpc_host"]

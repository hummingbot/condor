...
// (experiment = blue, run_once = amber). `badge` styles the ModeBadge pill;
// `text` is shared with compact renderings that only need the mode color.
export const MODE_STYLES: Record<string, { label: string; badge: string; text: string }> = {
  experiment: { label: "EXPERIMENT", badge: "border-blue-500/30 bg-blue-500/10 text-blue-400", text: "text-blue-400" },

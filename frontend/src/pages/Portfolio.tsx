...
function DonutChart({
  ...
  const arcs = slices.map((slice, i) => {
    ...
    return { ...slice, d, fraction, index: i };
  });
        ...
        <div className="flex flex-col gap-1.5 min-w-0 pt-1">
          {arcs.map((arc, i) => (
            ...
            >
              ...
              <span className="text-xs text-[var(--color-text-muted)] tabular-nums ml-auto pl-2">
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
      ...
      <div className="flex flex-col gap-2">
        {tokens.map((t, i) => {
          const pct = maxVal > 0 ? (t.usd_value / maxVal) * 100 : 0;
          return (
            <div key={`${t.connector}-${t.token}`} className="flex items-center gap-3">
              <span className="text-xs font-medium w-16 truncate text-right">{t.token}</span>
              ...
              <span className="text-xs tabular-nums text-[var(--color-text-muted)] w-20 text-right">
...
function PortfolioEvolution({ server }: { server: string }) {
  ...
  const minVal = values.length > 0 ? Math.min(...values) * 0.98 : 0;
        ...
        >
          ...
          {hover && (
            <>
              ...
              />
              {/* Tooltip */}
              <g transform={`translate(${hover.x < W / 2 ? hover.x + 12 : hover.x - 120}, ${PAD.top + 8})`}>
                <rect
                  width="108"
                  height="40"
                  rx="4"
...
export function Portfolio() {
  ...
  // Donut slices by top tokens (group small ones into "Other")
  const TOKEN_DONUT_LIMIT = 6;
  const tokenSlices: DonutSlice[] = [];
  let otherValue = 0;
  allTokens.forEach((t, i) => {
    if (i < TOKEN_DONUT_LIMIT) {
      tokenSlices.push({
        label: t.token,
        value: t.usd_value,
      ...
      otherValue += t.usd_value;
    }
  });
  if (otherValue > 0) {
    tokenSlices.push({ label: "Other", value: otherValue, color: "#52525b" });
  }
  ...
  return (
    <div className={`space-y-6 transition-opacity duration-300 ${isPlaceholderData ? "opacity-60" : "opacity-100"}`}>
      ...
      {/* Top Holdings + Token Allocation side by side */}
      {connectors.length > 0 && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {topTokens.length > 0 && (
            <TokenBarChart tokens={topTokens} title="Top Holdings" />
          )}
          <DonutChart
            slices={tokenSlices}
            title="Allocation by Token"
            centerLabel="Tokens"
            centerValue={String(totalTokens)}
          />
        </div>
      )}

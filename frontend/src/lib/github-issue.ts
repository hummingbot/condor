...
export function buildIssueUrl(draft: IssueDraft): string {
  ...
  params.set(fields.description, draft.description.trim());
  if (draft.detail?.trim()) params.set(fields.detail, draft.detail.trim());
  if (draft.diagnostics?.trim()) params.set(fields.extra, draft.diagnostics.trim());
  if (fields.logs && draft.logs?.trim()) params.set(fields.logs, draft.logs.trim());

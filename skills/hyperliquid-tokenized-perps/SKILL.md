...
description: "On hyperliquid_perpetual, tokenized perp contracts (equities/pre-IPO names like SPCX-USD) are issued by a provider and require an issuer prefix in the trading pair (e.g. XYZ:SPCX-USD, not SPCX-USD). Use when a user asks to trade / quote / deploy on a tokenized perp on hyperliquid_perpetual and the plain pair can't be found or looks unavailable — typically equity or pre-IPO names (e.g. SPCX, and other stock-like tickers) rather than crypto majors. Triggers — \"trade SPCX on hyperliquid\", \"can't find <TICKER>-USD on hyperliquid\", \"is <stock> available on hyperliquid perp\"; ES — \"no encuentro <TICKER>-USD en hyperliquid\", \"puedo operar <acción> en hyperliquid perp\"."
compatibility: "Requires the Condor MCP server (mcp__condor__* tools) connected"
metadata: {"condor-source": "builtin", "condor-created": "2026-07-02"}
---

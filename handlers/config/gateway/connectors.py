...
async def handle_connector_config_keep(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle keep current value button during connector configuration"""
    try:
        ...
        if current_index < len(all_fields) - 1:
            ...
            await submit_connector_config(context, query.message.get_bot(), query.message.chat_id)
...
def _build_connector_config_message(config_data: dict, current_field: str, all_fields: list) -> tuple:
    ...
    keep_buttons.append(InlineKeyboardButton(button_text, callback_data="gateway_connector_config_keep"))
    ...
    nav_buttons = []
    if current_index > 0:
        nav_buttons.append(InlineKeyboardButton("« Back", callback_data="gateway_connector_config_back"))

    # Always add cancel button
    nav_buttons.append(InlineKeyboardButton("❌ Cancel", callback_data=f"gateway_connector_cancel_edit_{connector_name}"))

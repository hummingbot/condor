...
FIELDS: Dict[str, ControllerField] = {
    ...
    "interval": ControllerField(name="interval", label="Candle Interval", type="str", required=True, hint="e.g. 1m, 5m, 1h, 8h", default="5m"),
...
def get_flat_fields(config: Dict[str, Any]) -> Dict[str, Any]:
    """Estrae solo i campi annidati (trailing_stop) in formato piatto."""
    trailing = config.get("trailing_stop", {})
    # Partiamo dai campi già presenti nel config (che sono già piatti)
    flat = dict(config)  # copia superficiale
    # Sostituiamo i due campi annidati con le loro versioni piatte
    ...
    # Rimuoviamo il dizionario originale perché non serve nella visualizzazione piatta
    flat.pop("trailing_stop", None)
    return flat


def apply_flat_fields(config: Dict[str, Any], updates: Dict[str, Any]) -> None:
    """Applica gli aggiornamenti, riconvertendo i due campi nel dizionario trailing_stop."""

...
async def show_executors_menu(
    ...
    try:
        ...
        keyboard.append(
            [
                InlineKeyboardButton("📜 History", callback_data="executors:history"),
...
async def show_history(
    ...
    """Show recent non-running (completed/stopped/failed) executors."""
    query = update.callback_query
    chat_id = update.effective_chat.id
    per_page = 8
    ...
    try:
        ...
        # Fetch non-running executors — the API doesn't have a "not RUNNING" filter,
        ...
        history.sort(key=lambda ex: ex.get("timestamp") or 0, reverse=True)

        # Cache for detail view
        context.user_data["history_executors"] = history
        ...
        lines = [
            f"📜 *Executor History* \\| _{escape_markdown_v2(server_name)}_",
        ]
        ...
        if page_items:
            ...
            lines.append("")
            lines.append("_No executor history found\\._")

        # Build keyboard
        keyboard = []

        # Executor detail buttons (2 per row)
        if page_items:
            row = []
            for ex, ex_type in displayed:
                executor_id = ex.get("id", ex.get("executor_id", ""))
                ...
                row.append(
                    InlineKeyboardButton(
                        ...
                        callback_data=f"executors:hist_detail:{executor_id[:20]}",
                    )
                )
        ...
        if total > per_page:
            ...
            if page > 0:
                nav_row.append(
                    InlineKeyboardButton(
                        "◀️ Prev", callback_data=f"executors:history:{page - 1}"
                    )
                )
            if start + per_page < total:
                nav_row.append(
                    InlineKeyboardButton(
                        "Next ▶️", callback_data=f"executors:history:{page + 1}"
                    )
                )
            if nav_row:
        ...
        keyboard.append(
            [
                InlineKeyboardButton("🔄 Refresh", callback_data="executors:history"),
        ...
        logger.error(f"Error showing executor history: {e}", exc_info=True)
        keyboard = [
            [
                InlineKeyboardButton("🔄 Retry", callback_data="executors:history"),
                InlineKeyboardButton("⬅️ Back", callback_data="executors:menu"),
            ]
        ]
        if query:
            try:
                await query.message.edit_text(
                    format_error_message(f"Failed to fetch history: {str(e)}"),
                    parse_mode="MarkdownV2",
                    reply_markup=InlineKeyboardMarkup(keyboard),
                )
            except BadRequest:
...
async def show_history_detail(
    update: Update, context: ContextTypes.DEFAULT_TYPE, executor_id: str
) -> None:
    """Show detail for a historical executor, with Back → History."""
    query = update.callback_query

    # Try to find in cached history first
    history = context.user_data.get("history_executors", [])
    executor = None
    for ex in history:
    ...
    if not executor:
        ...
        try:
            ...
            logger.warning(f"Could not fetch historical executor {executor_id}: {e}")
    ...
    # Store and render using the standard detail view logic,
    # but we override the keyboard to go back to history
...
async def _render_executor_detail(
    update: Update,
    ...
    leverage = config.get("leverage", 1)
    full_id = executor.get("id", executor.get("executor_id", ""))
    ...
    # Build keyboard — only show Stop for running executors
    keyboard = []
    is_running = status.upper() == "RUNNING"

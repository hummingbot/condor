...
async def token_login(req: TokenLoginRequest, response: Response):
    """Login using a one-time token from the /web Telegram command, or a
    short-lived token minted on the box by `condor login-token`."""
    ...
    if info is None:
        info = redeem_cli_login_token(req.token)
    if info is None:
        raise HTTPException(

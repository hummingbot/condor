...
async def get_routine_reports(
    ...
    allowed_names = {routine_name, base_name}
    # Filter by exact source_name first so high-volume routines get their newest runs,
    # not a search-limited slice that may miss recent reports for this routine.
    all_entries, _ = list_reports(source_type="routine", limit=2000)
    exact = [r for r in all_entries if r.get("source_name") in allowed_names]
    exact = exact[:limit]

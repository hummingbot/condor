...
async def _planned_amount_quote(input_data: dict[str, Any], client: Any) -> float:
    ...
    if executor_type == "dca_executor":
        ...
        # A declaration is not a spending limit. Native value also cannot bound
        # token transfers, approvals, or contract side effects. Until the API
        # enforces a valued bundle, only explicit dry runs may run unattended.
        if config.get("commit") is not False:
            raise ValueError(
                "On-chain exposure cannot be verified; automatic execution requires commit=false"
            )
        return 0.0

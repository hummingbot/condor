...
- [ ] Un único helper exportado produce el agrupado `[key, ExecutorInfo[]][]` por `connector:trading_pair`, salteando executors sin `trading_pair`
- [ ] `AgentDetail.tsx`, `AgentOverviewTab.tsx` y `AgentSessionContent.tsx` llaman al helper en vez de inlinear el loop del Map
- [ ] Los charts de executors vivos en las vistas de agent detail/overview/session renderizan los mismos grupos que antes

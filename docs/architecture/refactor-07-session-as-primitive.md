# Refactor 07 — consult / delegate / session: three verbs, three artifacts
...
## 2. The re-carve: three primitives, three artifacts

An agent's brain is invoked in exactly three ways, and each leaves exactly
...
| **delegate** | "go do this, tell me when done" | No (one-shot) | risk_gate (zero-seeded) / AUTO | a **transcript** — flat file, `delegations/{task_id}.md` |
...
**Session becomes the third primitive, replacing "tick" in the vocabulary.**
A session is the unit of stateful engagement: frozen config, journal, risk
state carried across invocations, `controller_id` attribution, a place in
the track record. *Ticking* is demoted to what it actually is — the
scheduler's way of advancing a session — not the identity of the thing.
(`max_ticks: 1` "run once" reads naturally now: a session advanced exactly
once.)
...
agents/{slug}/
    ...
    delegations/                     <- flat transcripts, one file per task
        {slug}-d{N}.md                  (header: status, task, risk_limits,
                                        started/ended; body: full transcript
                                        + result). Written AT START with
                                        status: running and rewritten on
                                        completion — keeps the crash-husk
                                        property without session machinery.
    curation/                        <- flat transcripts of curation passes,
        pass-{N}.md                     keep last ~10. The real audit trail
                                        stays the scoped git commit.
    dry_runs/experiment_N.md         <- unchanged
```
...
- **Delegation ids** get their own namespace again (`{slug}-d{N}`,
  monotonic per agent — readable, no uuid soup) and stop consuming session
  numbers.
...
2. **Uniform session analytics.** "Show me everything this agent did" is no
   longer one list — it is sessions + delegations dir + curation dir. The
   agent detail page still shows all three; they're just three sources.
...
going forward; less machinery; the three verbs finally map one-to-one onto
three artifacts a user can name.
...
2. For every `sessions/session_N` with `kind: delegation|consult|curation`:
   move `transcript.md` + meta header → `delegations/{slug}-d{K}.md` /
   `curation/pass-{K}.md` (consults: drop, or fold into `consults.log` if
   the option is adopted); delete the session dir.
3. **Never renumber surviving tick sessions** — `controller_id` tags on
...
4. Drop `kind:` from surviving tick-session metas.
...
4. Delegation transcript cap: recommend none for now (they are the durable

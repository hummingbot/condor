...
A Condor Vault is a **wallet that trades a strategy and is owned by a program,
...
just that person's wallet with extra structure. If they choose, they
**tokenize** it: a fixed-supply token is sold on a Meteora bonding curve, the
raise becomes the vault's capital, and from that moment **nobody — the runner,
the delegate, Condor — can move a token out of the wallet to any key**. The
wallet can still trade freely on allowed venues, including pools created
after launch, because the program checks *recipients* structurally instead of
keeping a list. The runner can pause, change the strategy, and wind the vault
down once; after a wind-down, holders burn tokens for a pro-rata share of the
quote asset, paid by the program itself.
...
|---|---|---|
| `Vault` | `["vault", id]` | The record and the vault's public identity. Runner, strategy pin, launch terms, state, delegate, mint. |
| wallet | `["vault_authority", id]` | A system account with no data. Holds the SOL and **owns every token account and LP position**. Also the DBC pool creator after tokenize. |
...
There is no key that can reach inside either vault account. The wallet signs
...
```
                 ...
                 └── ["vault_authority", id]  → wallet (money, ATAs, positions)
                                                 │
...
wallet's signing seeds without a `find_program_address` search; the wallet
...
|---|---|---|---|
| **runner** | the wallet that called `create_vault` | pin / publish the strategy, pause and resume, install or replace the delegate, act for the wallet, claim creator income, tokenize once, wind down once | after tokenize: move any token to any key |
| **delegate** | one key named on the `Vault` (Condor's crank key by default; the runner may install their own) | act for the wallet through `execute*` | change anything on the `Vault`; anything `execute` refuses |
...
The runner and the delegate have identical power over the *wallet*; the
difference is only that the runner also governs the `Vault`. That is
deliberate: what a key may do is decided by `execute`, not by whose key it is,
so a runner installing their own key on a tokenized vault costs holders
nothing.
...
create_vault ─► [private, Running] ⇄ Paused
               ...
               [tokenized, Running] ⇄ Paused
                      │ wind_down (one way; runner, or protocol authority)
                      ▼
...
* is emptied by its runner through `execute_unchecked` — a transfer is an
...
`Running` and `Paused` are the only states in which the wallet acts. Pausing
...
## 5. How the wallet acts: `execute_unchecked` and `execute`

Anything the wallet does is a CPI the program makes on its behalf. Both
...
The program sets the wallet's signer flag and `invoke_signed`s. Guards common
to both: signer is runner or delegate; state is `Running | Paused`; target is
executable and **is not this program** (a wallet that could call
`install_delegate` or `tokenize` as the runner could rewrite who owns it).
...
only its *mint* (verified in Meteora DLMM's `swap.rs`), so a wallet that may
...
|---|---|---|
| the wallet itself | allowed (rent it pays, lamports it receives) | — |
| a signer (the caller) | allowed (their own lamports) | — |
| a token account owned by the wallet | allowed | — |
...
| an **uninitialised, zero-lamport** system account | recorded as *fresh* | must now be a token account owned by the wallet, a DLMM `PositionV2` whose owner field is the wallet, or venue-owned — else `RecipientNotVault` |
| anything else — notably a token account owned by a key, or a *funded* system account (somebody's wallet) | `AccountNotAllowed` | — |

Non-venue programs are admitted only for what a trading wallet needs from
them, and **a token transfer is absent on purpose**:

| Program | Allowed under `execute` |
|---|---|
| Associated Token | `Create` / `CreateIdempotent` with owner == wallet |
| Token / Token-2022 | `SyncNative` on a wallet-owned account; `CloseAccount` of a wallet-owned account with rent to the wallet |
| System | `Transfer` only wallet → a wallet-owned token account (wrapping SOL); `CreateAccount` only when the payer is a signer other than the wallet |
...
Each economic term is a **bound** (the runner's decision, copied onto the
...
| creator's share of migration fee | constant | 100 % — the creator is the vault's own PDA, so nothing routes to the partner |
...
| leftover receiver | constant | the vault's wallet |
...
* **`collect_seed`** (permissionless): the creator's migration fee — the
  vault's capital — moves from DBC into the wallet's quote ATA. The creator is
  a PDA, so the destination is fixed and anybody may trigger it; Condor's
  promptness is irrelevant.
* **`collect_leftover`** (permissionless): the unsold supply moves into the
  wallet. It is the **treasury**, and it lives in the wallet on purpose: the
...
* **`claim_income` / `claim_position_fee`**: the *runner's* income — curve
  trading fees, the surplus above the threshold, and the locked migrated
  position's fees. Every destination is derived from `Vault.runner`; these
  are the only doors out of the creator's streams. The *vault's* income (LP
  fees from the strategy) arrives in the wallet directly.
...
1. **`wind_down`** — runner, or protocol authority for an abandoned vault.
...
2. **Conversion** — through `execute`, by the crank or anyone the vault lets
   ...
   Nothing moves anywhere; the wallet's own quote ATA *is* the redemption pot,
   because the wallet is the program's PDA and signs the payout itself.
...
4. **`redeem(amount)`** — any holder, any time after. Burns `amount` and pays
   ...
   vault balance − treasury (the wallet's own balance of its token)**. The
   pool's tokens are permanently locked liquidity; the treasury's were never
...
```
browser (ConnectorKit wallet) ── signs runner builds
   │
Condor web (FastAPI :8088)  /api/v1/vaults  ── record store, config hash, crank
   ...
   │                 (agents' executors build for walletAddress = the vault wallet)
   ▼
surfpool / Solana ◄── Gateway signs: runner builds are returned unsigned;
...
* **Wallet type `vault`.** `getWalletType` returns it for any address in
...
* **Vault routes** (`/chains/solana/vaults`): `GET /`, `GET /:vaultAccount`,
  ...
  `finalize-wind-down`. The platform key is Gateway's default Solana wallet,
...
* `src/vaults/vault-program.ts` is the typed client over the vendored IDL:
  ...
  `wallet` from the stored id), and one builder per instruction.

### 8.2 Condor (`condor/`)

* **Record store** (`vault_store.py`): one JSON per user keyed by the Vault
  PDA — label, server, `vault_id`, wallet and runner addresses, the private
...
* **Config commitment.** What goes on chain is the public agent folder by
  ...
  included, can run a vault on parameters its runner did not sign.
* **Web routes** (`condor/web/routes/vaults.py`, `/api/v1/vaults`): create /
  confirm, the runner builds, `withdraw`, `holdings` and `lp-positions`
...
* **Crank** (`vaults_crank.py`, per server): reads every vault from Gateway,
  keeps an hbapi account per vault wallet, starts or stops the strategy engine
  as state demands, tops up the delegate's gas from the wallet, calls the
...
runner), Create Vault, Vault detail with public Summary / Portfolio (Solana
tokens + LP positions of the wallet) / Activity tabs and runner-gated Agent
...
wallets. Runner actions are gated on `useCanSign` — the connected wallet
must be the runner.
...
* **A runner** of a private vault trusts nobody: it is their wallet, their
...
* **Condor** is trusted with nothing it can steal. Its crank's key is a
  ...
  wind-down, and only once the wallet is already in the quote asset.
...
| `create_vault(id, fund_lamports)` | runner | — | `Vault` + funded wallet (≥ rent floor) |
| `install_delegate(key)` | runner | not Redeemable | sets the delegate; no co-signer |
| `pin(agent_ref, config_hash)` | runner | version 0 | strategy v1, starts Running |
| `publish_version(...)` | runner | Running / Paused | new strategy version |
| `set_active(bool)` | runner | Running / Paused | pause / resume |
| `execute_unchecked(data)` | runner or delegate | private, Running / Paused | wallet invokes anything |
| `execute(data)` | runner or delegate | Running / Paused | wallet invokes a venue under the recipient rule |
| `tokenize(name, symbol, uri, issue_bps)` | runner | private | DBC launch, wallet as creator; writes quote and terms |
| `collect_seed` | anyone | migrated | creator's migration fee → wallet |
| `collect_leftover` | anyone | migrated | unsold supply → wallet |
| `claim_income(source)`, `claim_position_fee` | runner | tokenized | creator streams → runner |
| `wind_down` | runner, or protocol authority | tokenized, Running / Paused | one-way stop |
...
vault wallet landing through the wrap path, and the honest-vs-redirected DLMM

...
| 23 | **2026-09-18.** `PROTOCOL_MIGRATION_FEE_PCT = 1`: the creator's share of the migration fee is 99 %, the other 1 % is DBC's partner share, held for `Protocol.fee_claimer` and claimed by the platform (`claim-protocol-migration-fee`, cranked beside `collect-seed`) | "use this to define a protocol_graduation_fee so that 1% of the vault capital amount goes to the protocol" — the one platform fee, taken once, visible on chain before anyone buys |
| 24 | **2026-09-18.** Launch vocabulary: `protocol_graduation_fee_pct = 2` (was 1, and briefly "migration"); `migration_fee_pct` → **`locked_liquidity_pct`** (the share of the raise permanently locked; DBC keeps the complement as its migration fee, `tokenize` converts); `migration_fee_option` → `pool_fee_option`; our "migration" → "graduation" everywhere (`GRADUATION_QUOTE_THRESHOLD`, `graduationMarketCap`, `PoolNotGraduated`), DBC's own field names untouched in `dbc.rs` | "make it 2% by default. change the existing migration_fee_pct to locked_liquidity_pct … rename migration to graduation" — the creator chooses how deep the market is, and the rest is the strategy's |

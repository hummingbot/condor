...
Width is hard-capped by bin/tick granularity — exceed it and the open fails on-chain (no funds move). **Meteora DLMM ~69 bins max** (`bin_step=4` ⇒ total width ≲ 2.7%); Orca/Raydium bounded by `tick_spacing`. Pull `bin_step`/`tick_spacing` from `get_pool_info`, derive width from OHLCV volatility, then take `W = min(that, W_max)` — **the granularity cap is a CEILING on the width, not a follow-up check** (a `bin_step=10` Meteora pool caps `W_max` at 3.40%, below any sane volatility floor). (Details and the `W_max` table: `lp_range_config` skill.)
...
## Response format
When handed a question, respond with key: value lines: lead with the recommendation (pool, venue, side, range, size, or hold/exit), then brief reasoning.

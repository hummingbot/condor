...
- **Orca / Raydium** — `spacings = ln(upper/lower) / (ln(1.0001) × tick_spacing)` must be **≤ 120**; table at **119 spacings**, one spacing under the cap. The values are exact ceilings: read them as written, never round the last decimal up, or the clamp lands back on the cap:

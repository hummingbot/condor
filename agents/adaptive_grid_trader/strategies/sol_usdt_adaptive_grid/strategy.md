...
- **`risk_envelope` is a mandatory step before every deploy** (new step 6).
  It replaces the hand-rolled liq guard and range math.
- **Profit-taking is arm-and-ride on *realized* PnL**, not a 2% close on blended
  `net_pnl_quote`. Unrealized goes negative exactly when a grid is working, so
  the old rule fired on noise and capped winners.
- **`stop_loss 0.10` removed — the grid executor has no such parameter.** It
  never did anything. Real exits are `limit_price`, `time_limit` and teardown.
...
- max_loss_pct: 10% ($10)
- session_max_loss_pct: 15% ($15 cumulative realized → circuit breaker)
...
|---|---|
| Session loss | cumulative realized ≤ **−$15** (15% of budget) |
...
- For TWO_SIDED: check each leg independently, same two-tick rule per leg
...
1. **Arm** when `realized` ≥ **2% of trade budget** ($1.80 one-sided, $0.90/leg on TWO_SIDED). Do not close. Record `peak_realized`.
...
3. **Close** on the first of:
   ...
   - **Ceiling** — `realized` ≥ **5% of budget** ($4.50 one-sided)
   - **Regime** — baseline flips against the grid
4. **Give-back floor** — once armed, never exit below **1.5% net** ($1.35). If give-back would fire below that, hold and let stall / ceiling / regime / `limit_price` / circuit breaker decide.
...
- For TWO_SIDED: per-leg. One leg closing does not touch the other
...
5. **TWO_SIDED collapse:** both TF lock one way → teardown both → one-sided next tick
...
manage_routines(action="run", name="risk_envelope",
  agent="adaptive_grid_trader",
  config={"trading_pair":"SOL-USDT","connector_name":"binance_perpetual",
          "side":"<LONG|SHORT>","budget":100,"reserve_pct":0.10,
...
`total_amount_quote` is **notional**, not the $100 budget — it is
`budget × (1 − reserve) × leverage`. Journal the margin and worst-case loss beside it.

### 7. Deploy grid_executor
Use the envelope's numbers verbatim. Fixed on this strategy: min_order 7;
max_open_orders 12; activation_bounds 0.002; time_limit 43200; keep_position
false; `open_order_type=3` (LIMIT_MAKER); `take_profit_order_type=3`;
`coerce_tp_to_step=True`; controller_id = this session agent_id.
...
### 8. Journal
entry_path, mode (HEDGE|ONEWAY), mode_read if present, two_sided_allowed,
baseline, 4h/1d, `risk_envelope` verdict, derived leverage, liq_guard,

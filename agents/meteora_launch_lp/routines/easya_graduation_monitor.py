...
class Config(BaseModel):
    ...
    max_age_hours: float = Field(
        default=72.0, description="Only pools created within this many hours"
    )
    min_tvl_usd: float = Field(
        default=10000.0,
        description="Minimum pool TVL in USD (graduation liquidity floor)",
    )
    min_vol24h_usd: float = Field(
        default=3000.0, description="Minimum 24h volume in USD (real post-grad demand)"
    )
    verified_only: bool = Field(
        default=False, description="Require the graduated token to be verified"
    )
    require_static_fee: bool = Field(
        default=True, description="Exclude fee-scheduler pools (EasyA grads are static)"
    )
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                DAMM_V2_API, params=params, timeout=aiohttp.ClientTimeout(total=25)
            ) as resp:
                if resp.status != 200:
    ...
    for p in payload.get("data", []):
        ...
        if config.require_static_fee and (
            cfg.get("is_fee_scheduler_active") or cfg.get("has_fee_scheduler")
        ):
            continue
        ...
        candidates.append(
            {
                ...
                "token_symbol": easy_side.get("symbol")
                or easy_side.get("address", "")[:6],
    ...
    if not candidates:
        return (
            f"easya_graduation_monitor: no EasyA graduations in the last {config.max_age_hours:.0f}h "
    ...
    try:
        ...
        for r in rows:
            lines.append(
                f"{r['#']}. {r['Pair']} | age {r['Age(h)']}h | yield {r['FeeYield']} | "

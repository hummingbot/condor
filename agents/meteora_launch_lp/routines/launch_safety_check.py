...
class Config(BaseModel):
    ...
    require_verified: bool = Field(default=False, description="Fail if the token is not verified")
    min_lock_pct: float = Field(default=0.0, description="Require locked+vested liquidity ≥ this % of TVL (0 = off)")
    min_tvl_usd: float = Field(default=10000.0, description="Fail below this TVL")


async def _rpc(session, url, method, params):
    async with session.post(url, json={"jsonrpc": "2.0", "id": 1, "method": method, "params": params},
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(DAMM_V2_API, params={"query": config.pool_address, "page_size": 1},
    ...
    gates.append(("tvl", tvl >= config.min_tvl_usd, f"${tvl:,.0f} (min ${config.min_tvl_usd:,.0f})"))
    if config.require_verified:
        gates.append(("verified", bool(base.get("is_verified")), f"{base.get('symbol')} verified={base.get('is_verified')}"))
    if config.require_freeze_disabled:
        gates.append(("freeze_disabled", bool(base.get("freeze_authority_disabled")),
    ...
    if config.min_lock_pct > 0:
        ...
        vested = sum(float(v or 0) for v in (pool.get("vested_liquidity") or {}).values())
        lock_pct = (locked + vested) / tvl * 100 if tvl else 0.0
        gates.append(("lp_lock", lock_pct >= config.min_lock_pct, f"{lock_pct:.1f}% locked/vested (min {config.min_lock_pct}%)"))

    # 2. On-chain gates via RPC: mint authority + top-holder concentration.
    try:
        async with aiohttp.ClientSession() as session:
            if config.require_mint_renounced:
                info = await _rpc(session, config.rpc_url, "getAccountInfo",
                                  [base_mint, {"encoding": "jsonParsed"}])
                parsed = ((info or {}).get("value") or {}).get("data", {}).get("parsed", {}).get("info", {})
                mint_auth = parsed.get("mintAuthority")
                gates.append(("mint_renounced", mint_auth is None, f"mintAuthority={mint_auth or 'null (renounced)'}"))

            supply_res = await _rpc(session, config.rpc_url, "getTokenSupply", [base_mint])
            total = float((supply_res or {}).get("value", {}).get("uiAmount") or 0)
            largest = await _rpc(session, config.rpc_url, "getTokenLargestAccounts", [base_mint])
            ...
            gates.append(("holder_concentration", top10_pct <= config.max_top10_holder_pct,
    ...
    lines = [f"Safety check for {base.get('symbol','?')} ({base_mint}) — pool {config.pool_address}",
             f"VERDICT: {'PASS' if passed_all else 'FAIL'}", ""]
    for name, ok, detail in gates:
        lines.append(f"  [{'PASS' if ok else 'FAIL'}] {name}: {detail}")
    lines.append("")
    lines.append("NOTE: sellability (honeypot) is NOT checked here — round-trip a SELL and BUY quote via "

...
class Config(BaseModel):
    ...
    perp_instrument: str = Field(default="SOL-PERP", description="Perpetual instrument to signal")
    rr_weight: float = Field(default=0.50, description="Weight for 25D Risk Reversal (0–1)")
    oi_weight: float = Field(default=0.35, description="Weight for Put/Call OI Ratio (0–1)")
    ts_weight: float = Field(default=0.15, description="Weight for Term Structure (0–1)")
    min_threshold: float = Field(default=0.40, description="Min |composite score| to act; below = HOLD")
...
async def _fetch_instruments(session: aiohttp.ClientSession, currency: str) -> list[dict]:
    data = await _post(session, "public/get_instruments", {
        "currency": currency, "instrument_type": "option", "expired": False
    })
    return data.get("result") or []


async def _fetch_tickers(session: aiohttp.ClientSession, currency: str, expiry_date: str) -> dict:
    data = await _post(session, "public/get_tickers", {
        "currency": currency, "instrument_type": "option",
        "expired": False, "expiry_date": expiry_date,
    })
...
def _compute_25d_rr(opts: list[dict]) -> tuple[float | None, str, str]:
    ...
    calls = [o for o in opts if o["type"] == "C" and o["iv"] > 0 and abs(o["delta"]) > 0.01]
    puts  = [o for o in opts if o["type"] == "P" and o["iv"] > 0 and abs(o["delta"]) > 0.01]
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    def dte(exp_str: str) -> int:
        return max(1, (datetime.strptime(exp_str, "%Y%m%d").replace(tzinfo=timezone.utc) - today).days)
    ...
    for exp in sorted(expiry_opts):
        ...
        exp_rows.append({
            ...
            "Bias": ("BULL" if rr_v and rr_v > 0 else "BEAR") if rr_v is not None else "—",
            "Call OI": f"{c_oi:.0f}",
            "Put OI": f"{p_oi:.0f}",
            "P/C Ratio": f"{p_oi/max(c_oi,1):.2f}",
        })

    builder.section("PER-EXPIRY BREAKDOWN", "Signals across all active option expiry dates")
    builder.table(exp_rows, ["Expiry", "DTE", "ATM IV", "25D RR", "Bias", "Call OI", "Put OI", "P/C Ratio"])

    # IV surface for nearest liquid expiry
    if gex_exp:
        surf_opts = [
            o for o in expiry_opts[gex_exp]
        ...
        surf_rows = sorted([{
            ...
            "OI": f"{o['oi']:.0f}",
            "Vol 24h": f"{o['vol']:.1f}",
        } for o in surf_opts], key=lambda x: (x["Strike"], x["Type"]))

        builder.section(f"IV SURFACE — {gex_exp}", "Options within ±30% of spot, nearest liquid expiry")
    ...
    return RoutineResult(
        ...
        sections=[
            ...
            {"type": "kpi", "label": f"{config.currency} Spot", "value": f"${spot:.3f}"},

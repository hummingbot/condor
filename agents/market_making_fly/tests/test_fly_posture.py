...
def test_the_exit_scales_with_the_market_and_never_goes_under_the_fee():
    """The take-profit was the fee floor whether a market moved 2 bp a bar or
    20, so five replay variants closed the same eleven round trips. It is now
    three quarters of a typical bar, with the fee floor and the fly's own first
    level underneath it.

    The fly does not move it. Letting arousal widen it halved the round trips —
    35 against a fixed exit's 68 on the same candles — and earned nothing for
    them, so that half of the change was reverted and this is the half kept."""
    ...
    # SPEC's 10 bp range puts the exit at 7.5 bp, whatever the posture says
    for posture in (
        NEUTRAL,
        Posture("volatile", 1.0, 1.0, 0.0, 0, 1.5, 0.0, False, True),
        Posture("quiet", 0.6, 1.0, 0.0, 0, -1.5, 0.0, False, True),
    ):
        assert float(build_config(SPEC, posture)["take_profit"]) == pytest.approx(
            7.5 * BPS
        )

    # and on a market that barely moves, the fee floor holds the exit up
    tight = MarketSpec(**{**SPEC.__dict__, "range_bps": 2.0})
    cfg = build_config(tight, NEUTRAL)
...
def test_a_gated_trend_takes_a_side_off_the_book():
    ...
    up = build_config(
        SPEC,
        Posture("trending_up", 1.0, 1.0, 2.0, 2.0, 0, 0.0, True, True, "buy"),
    )
    assert up["sell_amounts_pct"] == "0,0" and up["buy_amounts_pct"] == "1,1"
    down = build_config(
        SPEC,
        Posture("trending_down", 1.0, 1.0, -2.0, -2.0, 0, 0.0, True, True, "sell"),
    )

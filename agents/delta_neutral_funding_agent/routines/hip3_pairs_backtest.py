...
class Config(BaseModel):
    ...
    mode: str = Field(
        default="pair",
        description="'pair' = single-pair spread backtest; 'universe' = full HIP-3 universe L/S",
    )
    # ── universe mode ─────────────────────────────────────────────────────────
    issuer: str = Field(
        default="xyz", description="[universe] HIP-3 builder issuer slug (e.g. 'xyz')"
    )
    trend_lookback_hours: int = Field(
        default=24, description="[universe] Momentum lookback for trend score/ranking"
    )
    rebalance_hours: int = Field(
        default=6, description="[universe] How often to re-rank and rebalance the book"
    )
    top_k: int = Field(
        default=1, description="[universe] Legs per side (1 = single long/short pair)"
    )
    min_volume_usd: float = Field(
        default=1_000_000.0, description="[universe] Min 24h volume filter (dayNtlVlm)"
    )
    min_oi_notional: float = Field(
        default=1_000_000.0,
        description="[universe] Min OI filter (openInterest * mark)",
    )
    taker_bps_per_side: float = Field(
        default=3.5, description="[universe] Taker fee per side for rotations in bps"
    )
    include_funding: bool = Field(
        default=True,
        description="[universe] Include perp funding payments on held legs",
    )
    momentum: bool = Field(
        default=True,
        description="[universe] True=long winners/short losers; False=reversion",
    )
    # ── pair mode ─────────────────────────────────────────────────────────────
    pair_long: str = Field(
        default="XYZ100",
        description="[pair] Long leg coin (xyz: prefix added automatically)",
    )
    pair_short: str = Field(
        default="SP500",
        description="[pair] Short leg coin (xyz: prefix added automatically)",
    )
    hedge_window: int = Field(
        default=168,
        description="[pair] Rolling window (bars) for hedge ratio AND z-score (168h = 7d)",
    )
    signal: str = Field(
        default="reversion",
        description="[pair] Primary signal: 'reversion' or 'momentum' (both always reported)",
    )
    entry_z: float = Field(
        default=1.5, description="[pair] Entry z-score threshold (reversion)"
    )
    exit_z: float = Field(
        default=0.3, description="[pair] Exit z-score threshold (reversion)"
    )
    momentum_lookback: int = Field(
        default=48, description="[pair] Lookback bars for momentum signal"
    )
    # ── shared ────────────────────────────────────────────────────────────────
    ...
    fee_bps_per_side: float = Field(
        default=1.3, description="All-in maker fee per side in bps"
    )
        ...
        for j in range(n):
            if i == j:
                ...
                if stds[i] == 0 or stds[j] == 0:
                    ...
                    cov = sum(
                        (ri[k] - means[i]) * (rj[k] - means[j]) for k in range(len(ri))
                    ) / len(ri)
...
async def _fetch_candles(
    session, coin: str, interval: str, start_ms: int, end_ms: int
) -> list:
    payload = {
        "type": "candleSnapshot",
        "req": {
    ...
    try:
        async with session.post(
            HL_URL, json=payload, timeout=aiohttp.ClientTimeout(total=30)
        ) as resp:
            if resp.status != 200:
...
def _simulate(
    ...
    def _get_funding_for_period(
        coin: str, t_start: int, t_end: int, is_long: bool
    ) -> float:
    ...
    for step_i, start_idx in enumerate(
        ...
        if funding_included:
            ...
            for coin in longs:
                period_funding += leg_notional * _get_funding_for_period(
                    coin, t_start, t_end, True
                )
            for coin in shorts:
                period_funding += leg_notional * _get_funding_for_period(
                    coin, t_start, t_end, False
                )
    ...
    if prev_longs or prev_shorts:
        closing_fee = (
            (len(prev_longs) + len(prev_shorts)) * leg_notional * fee_bps / 10000
        )
    ...
    if n_steps == 0:
        return {
            "error": "no simulation steps — check lookback/rebalance vs available candles"
        }
...
def _simulate_pair(
    ...
    if start >= n - 1:
        raise ValueError(
            f"Insufficient data: {n} bars, need > {2 * W + 1} (2*hedge_window+1)"
        )
    ...
    baseline_rets = [
        (la[i + 1] - la[i]) - betas[i] * (lb[i + 1] - lb[i])
        for i in range(start, n - 1)
    ]
    ...
    baseline_short_ann = (
        baseline_short_net * bars_per_year / n_sim if n_sim > 0 else 0.0
    )
...
async def _run_pair_mode(config: Config) -> str:
    ...
    if not long_candles:
        raise ValueError(
            f"No candle data for {long_coin} — verify the coin name and that it trades on Hyperliquid HIP-3"
        )
    if not short_candles:
        raise ValueError(
            f"No candle data for {short_coin} — verify the coin name and that it trades on Hyperliquid HIP-3"
        )
    ...
    _, timestamps, price_matrix = _align_series(
        {long_coin: long_series, short_coin: short_series}
    )
    ...
    if not corr_pass:
        ...
        best = (
            "reversion"
            if (rev_go and (not mom_go or rev["ann_net"] >= mom["ann_net"]))
            else "momentum"
        )
        ...
        if rev["n_trades"] < 30 and mom["n_trades"] < 30:
            issues.append(
                f"too few trades (rev={rev['n_trades']}, mom={mom['n_trades']})"
            )
        ...
        if drift_flags:
            issues.append(
                "return captured by static baseline (in-sample drift, not alpha)"
            )
        verdict = (
            "NO-GO: "
            + ("; ".join(issues) if issues else "no edge after fees/flags")
            + ". NOT a tradeable spread."
        )
    ...
    summary = "\n".join(
        [
            ...
            "",
            "━━ VERDICT ━━",
            f"  Corr gate (≥{config.min_corr}):      "
            + ("PASS ✓" if corr_pass else f"FAIL ✗  ({result['ret_corr']:.3f})"),
            f"  Rev trades ≥ 30:      "
            + (
                "PASS ✓"
                if rev["n_trades"] >= 30
                else f"FAIL ✗  ({rev['n_trades']} trades)"
            ),
            f"  Mom trades ≥ 30:      "
            + (
                "PASS ✓"
                if mom["n_trades"] >= 30
                else f"FAIL ✗  ({mom['n_trades']} trades)"
            ),
            f"  Rev ann net > 0:      "
            + ("PASS ✓" if rev["ann_net"] > 0 else "FAIL ✗")
            + f"  ({rev['ann_net']:.1%}/yr)",
            f"  Mom ann net > 0:      "
            + ("PASS ✓" if mom["ann_net"] > 0 else "FAIL ✗")
            + f"  ({mom['ann_net']:.1%}/yr)",
            "",
            f"  >> {verdict}",
        ]
    ...
    builder.source("routine", "hip3_pairs_backtest").tags(
        [
            ...
            "hip3",
            "pair-backtest",
            config.pair_long.lower(),
            config.pair_short.lower(),
        ]
    )
    ...
    rows = [
        ...
        {
            "Metric": f"Ann Vol {config.pair_long}",
            "Value": f"{result['ann_vol_long']:.2%}",
        },
        {
            "Metric": f"Ann Vol {config.pair_short}",
            "Value": f"{result['ann_vol_short']:.2%}",
        },
    ...
    if eq_rev or eq_mom:
        ...
        fig.update_layout(
            ...
            xaxis_title="Bar (hourly)",
            yaxis_title="Cumulative Log-Return P&L",
...
async def _run_universe_mode(config: Config, context) -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                HL_URL,
                json={"type": "metaAndAssetCtxs", "dex": issuer},
    ...
    if len(tradable) < 2:
        return (
            f"Only {len(tradable)} tradable markets pass vol/OI filters "
    ...
    if len(tradable) < 2 * config.top_k:
        return (
            f"Only {len(tradable)} tradable markets but need {2 * config.top_k} "
    ...
    async def _bounded(session, coin):
        async with sem:
            return await _fetch_candles(
                session, coin, config.interval, start_ms, now_ms
            )
    ...
    if len(raw_series) < 2:
        return (
            f"Only {len(raw_series)} markets have usable candle data "
    ...
    if len(timestamps) < 20:
        return (
            f"Too few common timestamps ({len(timestamps)}) across markets "
        ...
        try:
            async with aiohttp.ClientSession() as session:
                fund_results = await asyncio.gather(
                    *[
                        _fetch_funding_history(session, coin, start_ms)
                        for coin in aligned_coins
                    ]
                )
    ...
    if not gate1_pass:
        ...
        if best_g2 and best_g3:
            verdict = (
                f"GO ({best}): correlated universe ✓ market-neutral ✓ "
            ...
            verdict = (
                f"CONDITIONAL ({best} shows edge but not all gates clear) "
    ...
    summary = "\n".join(
        [
            ...
            "━━ 2 & 3. SIMULATION ━━",
            _fmt(
                "MOMENTUM (long winners / short losers)", mom_result, mom_beta, mom_r2
            ),
            "",
            _fmt(
                "REVERSION (long losers / short winners)", rev_result, rev_beta, rev_r2
            ),
            "",
            "━━ VERDICT ━━",
    ...
    builder.kpi(
        ...
        (
            ...
            else ("NO-GO" if verdict.startswith("NO-GO") else "CONDITIONAL")
        ),
    )
    ...
    if _ok(mom_result) and _ok(rev_result):
        compare = [
            {
                "Metric": "Ann Net Return",
                "Momentum": f"{mom_result['ann_net_return']:.2%}",
                "Reversion": f"{rev_result['ann_net_return']:.2%}",
            },
            {
                "Metric": "Ann Gross Return",
                "Momentum": f"{mom_result['ann_gross_return']:.2%}",
                "Reversion": f"{rev_result['ann_gross_return']:.2%}",
            },
            {
                "Metric": "Sharpe",
                "Momentum": f"{mom_result['sharpe']:.3f}",
                "Reversion": f"{rev_result['sharpe']:.3f}",
            },
            {
                "Metric": "Max Drawdown",
                "Momentum": f"{mom_result['max_drawdown']:.2%}",
                "Reversion": f"{rev_result['max_drawdown']:.2%}",
            },
            {
                "Metric": "Hit Rate",
                "Momentum": f"{mom_result['hit_rate']:.1%}",
                "Reversion": f"{rev_result['hit_rate']:.1%}",
            },
            {
                "Metric": "Fee Drag",
                "Momentum": f"{mom_result['total_fees']:.5f}",
                "Reversion": f"{rev_result['total_fees']:.5f}",
            },
            {
                "Metric": "Funding",
                "Momentum": f"{mom_result['total_funding']:.5f}",
                "Reversion": f"{rev_result['total_funding']:.5f}",
            },
            {
                "Metric": "Residual β",
                "Momentum": f"{mom_beta:.4f}",
                "Reversion": f"{rev_beta:.4f}",
            },
            {
                "Metric": "Beta R²",
                "Momentum": f"{mom_r2:.3f}",
                "Reversion": f"{rev_r2:.3f}",
            },
            {
                "Metric": "Steps",
                "Momentum": str(mom_result["n_steps"]),
                "Reversion": str(rev_result["n_steps"]),
            },
        ]
    ...
    if (
        _ok(mom_result)
        and mom_result.get("index_returns")
        and mom_result.get("step_returns")
    ):
        ...
        if _ok(rev_result) and rev_result.get("index_returns"):
            fig2.add_trace(
                go.Scatter(
                    x=rev_result["index_returns"],
                    y=rev_result["step_returns"],
                    mode="markers",
                    name="Reversion",

...
class Config(BaseModel):
    ...
    leg_a: str = Field(
        default="CL", description="Coin A (xyz: prefix added automatically)"
    )
    leg_b: str = Field(
        default="BRENTOIL", description="Coin B (xyz: prefix added automatically)"
    )
    interval: str = Field(
        default="1h", description="Candle interval for beta/correlation window"
    )
    lookback_hours: int = Field(
        default=168, description="Lookback window in hours (7d) for beta + correlation"
    )
    configured_hedge_beta: float = Field(
        default=1.02, description="Configured hedge beta from strategy; compare vs live"
    )
    total_amount_quote: float = Field(
        default=500.0, description="Total gross notional (quote) to deploy"
    )
    net_delta_band_pct: float = Field(
        default=0.1, description="Net delta tolerance band as fraction of total"
    )
    min_corr: float = Field(default=0.9, description="Minimum return correlation gate")
    beta_drift_tol: float = Field(
        default=0.2,
        description="RESIZE trigger: |live_beta - configured| / configured > this",
    )
...
async def _fetch_candles(
    session: aiohttp.ClientSession, coin: str, interval: str, start_ms: int, end_ms: int
) -> list:
    payload = {
        "type": "candleSnapshot",
        "req": {
    ...
    async with session.post(
        ...
        if not isinstance(data, list):
            raise RuntimeError(
                f"Unexpected candleSnapshot response for {coin}: {type(data)}"
            )
        return data
...
async def _fetch_meta_and_ctxs(session: aiohttp.ClientSession) -> tuple:
    ...
    async with session.post(
        HL_URL, json=payload, timeout=aiohttp.ClientTimeout(total=15)
    ) as resp:
        if resp.status != 200:
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    # ── 3. Log returns + stats ─────────────────────────────────────────────────
    log_a = [
        math.log(prices_a[i] / prices_a[i - 1])
        for i in range(1, len(prices_a))
        if prices_a[i - 1] > 0 and prices_a[i] > 0
    ]
    log_b = [
        math.log(prices_b[i] / prices_b[i - 1])
    ...
    raw_positions = (
        await fetch_positions(client, connector_name="hyperliquid_perpetual")
        if client
        else []
    )
    ...
    beta_drift = abs(live_beta - config.configured_hedge_beta) / max(
        abs(config.configured_hedge_beta), 1e-10
    )
    ...
    actual_delta_breach = (
        has_live_position and abs(actual_net_factor_delta) > delta_band
    )
    ...
    rows = [
        ...
        {
            "Metric": f"Carry Config A ({config.leg_a}L/{config.leg_b}S)",
            "Value": f"{net_carry_hr_A*24*365*100:+.1f}%/yr",
        },
        {
            "Metric": f"Carry Config B ({config.leg_a}S/{config.leg_b}L)",
            "Value": f"{net_carry_hr_B*24*365*100:+.1f}%/yr",
        },
        {
            "Metric": "Recommended Config",
            "Value": f"Config {recommended_config}: {config_desc}",
        },
        {"Metric": "Net Carry (best)", "Value": f"{net_carry_yr:+.1f}%/yr"},
        {
            "Metric": f"Leg A ({config.leg_a}) TARGET Notional",
            "Value": f"${abs(signed_a):.2f} ({'L' if signed_a > 0 else 'S'}) [theoretical]",
        },
        {
            "Metric": f"Leg B ({config.leg_b}) TARGET Notional",
            "Value": f"${abs(signed_b):.2f} ({'L' if signed_b > 0 else 'S'}) [theoretical]",
        },
        {
            "Metric": "Net Factor Delta TARGET (theoretical)",
            "Value": f"{net_factor_delta:.4f}",
        },
    ...
    if not has_live_position:
        ...
        rows += [
            {
                "Metric": f"Leg A ({config.leg_a}) ACTUAL Notional",
                "Value": f"${signed_notional_a:+.2f} ({'L' if signed_notional_a >= 0 else 'S'}, amt={amount_a:+.6f})",
            },
            {
                "Metric": f"Leg B ({config.leg_b}) ACTUAL Notional",
                "Value": f"${signed_notional_b:+.2f} ({'L' if signed_notional_b >= 0 else 'S'}, amt={amount_b:+.6f})",
            },
            {
                "Metric": "Actual Net Factor Delta",
                "Value": f"${actual_net_factor_delta:+.2f} ({_abl})",
            },
        ]

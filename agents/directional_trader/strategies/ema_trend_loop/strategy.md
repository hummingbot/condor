...
## Objective
Run a portfolio of EMA trend-following controllers across any available pairs on binance_perpetual. Each tick: discover available configs, decide which ones to run, monitor open positions, manage risk, and journal learnings that inform the next tick's decisions.

The agent is NOT locked to specific pairs or configs. It discovers what's available, selects the best candidates based on backtest metrics and accumulated live experience, and evolves its selection over time.
...
From your journal learnings, recall which configs have performed well or poorly live. Prefer configs with:
- Sharpe > 1.0 in backtest
- No recent drawdown warnings in journal
- Not currently stopped for drawdown this session

### Step 2 — Decide active set
Select up to `max_pairs` configs to run. Prioritize based on:
1. Strong backtest metrics (Sharpe, max DD, win rate)
2. Positive or neutral live experience from journal
3. Diversity: avoid two configs on the same pair/timeframe if possible
...
- If NOT running and active set is non-empty → deploy:
  ...
  ```
  Journal: action="deployed ema_trend_loop with <configs>"
...
### Step 4 — Check positions and performance
...
### Step 6 — Journal state + learnings
Write a state entry:
- Bot status: running / stopped
- Active controllers and their positions (side, size, unrealized PnL or "flat")
- Total PnL this session

If you observed anything useful this tick (e.g. a config underperforming, a pair in a choppy regime, a config that caught a clean trend), write a learning entry. These learnings will inform Step 1–2 on future ticks.
...
2. Skip to Step 6 (state journal) — never crash the tick

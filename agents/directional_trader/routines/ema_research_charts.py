...
class Config(BaseModel):
    ...
    connector: str = Field(default="binance_perpetual", description="Exchange connector")
    pairs: str = Field(
        default="BTC-USDT,ETH-USDT,SOL-USDT",
        description="Comma-separated trading pairs",
    )
    timeframe: str = Field(default="15m", description="Candle interval (1m/3m/15m/1h/4h)")
    days: int = Field(default=30, description="Days of history to fetch")
    hold_bars: int = Field(default=5, description="Bars ahead to measure post-signal move")
    ...
    adx_filter: int = Field(default=0, description="Min ADX for a signal to count (0=disabled)")
...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    data = {}
    for pair in pairs:
        df = await _fetch_df(client, config.connector, pair, config.timeframe, config.days)
    ...
    for pair in active_pairs:
        ...
        dist_stats[pair] = {
            ...
            "regime": (
                "trend" if autocorr > 0.05 else ("mean-rev" if autocorr < -0.05 else "random")
            ),
        }
    ...
    best_per_pair = {
        pair: sweep_results[pair][0]
        for pair in active_pairs
        if sweep_results.get(pair)
    }
    current_signals = {
        pair: {
    ...
    for ci, pair in enumerate(active_pairs):
        ...
        ax_h.axvline(ret.mean(), color="white", linestyle="--", linewidth=1,
    ...
    for ci, pair in enumerate(active_pairs):
        ...
        ax.axvline(
            float(adx_s.median()), color="white", linestyle="-.", linewidth=1,
    ...
    for ri, pair in enumerate(active_pairs):
        ...
        ax = axes[ri, 0]
        _style_ax(ax)
        ax.plot(df_c["datetime"], df_c["close"], color="white", linewidth=0.6,
                label="Close", zorder=2)
        ax.plot(df_c["datetime"], df_c["ema_f"], color="#3b82f6", linewidth=1.3,
                label=f"EMA{fp}", zorder=3)
        ax.plot(df_c["datetime"], df_c["ema_s"], color="#f59e0b", linewidth=1.3,
                label=f"EMA{sp}", zorder=3)

        longs = df_c[df_c["cross"] & (df_c["sig"] == 1)]
        shorts = df_c[df_c["cross"] & (df_c["sig"] == -1)]
        ax.scatter(longs["datetime"], longs["close"], marker="^", color="#22c55e",
                   s=50, zorder=5, label="Long")
        ax.scatter(shorts["datetime"], shorts["close"], marker="v", color="#ef4444",
                   s=50, zorder=5, label="Short")
    ...
    if len(results_hm) > 1:
        ...
        if best_hm:
            ...
            if bfi is not None and bsi is not None:
                ax.add_patch(
                    plt.Rectangle((bsi - 0.5, bfi - 0.5), 1, 1, fill=False,
                                  edgecolor="white", linewidth=2)
                )
                ax.text(bsi, bfi, "★", ha="center", va="center",
    ...
    rb.table(
        top3,
        ["pair", "fast", "slow", "score", "win_rate", "sharpe",
         "avg_persistence", "signals_per_day", "current_signal"],
    )
    if top3:
        rb.markdown(f"## Recommendation\n\n{lines[-1]}")
    rb.tags(["ema", "research", "trend"] + [p.split("-")[0].lower() for p in active_pairs])
    await rb.save()

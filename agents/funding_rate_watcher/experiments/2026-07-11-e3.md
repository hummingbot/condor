# Experiment #3 — 2026-07-11 17:47 UTC
Mode: dry_run
Model: claude-acp:sonnet

<details><summary>System Prompt (4702 chars)</summary>
...
Agent ID: funding_rate_watcher_e3
...
[CORE DATA - executors]
Active Executors: none running (agent: funding_rate_watcher_e3)
  Realized: $+0.00 | Unrealized: $+0.00 | Total PnL: $+0.00 | Volume: $0

[CORE DATA - positions]
Positions Summary [agent: funding_rate_watcher_e3]: no open positions
...
**Funding Rate Watch — Tick #1 | SOL-USD on Hyperliquid Perpetual**

**Rate:** `+0.0100%` per 8h (longs pay shorts)
**Mark Price:** `$78.063`
**Classification:** `ELEVATED`

**Read:**
The 8h funding rate sits at exactly the baseline threshold of +0.01%. This is the boundary between neutral and elevated — longs are paying shorts at a rate that annualizes to roughly **+10.95% APR**. It's not extreme, but it's not free either.

**Implications:**
- Long bias is present in the market but not overheated. Shorts earn carry at a modest but meaningful rate.
- No squeeze signal — funding this mild rarely precedes a violent long squeeze.
- If funding were to climb toward +0.05–0.10%/8h, that would signal crowded longs and meaningful squeeze risk.

**What I would do (🧪 dry run):**
- Would continue monitoring without action at this level.
- 🧪 If funding crossed +0.03%/8h, would flag as extreme and alert the user to consider a carry short or risk-off posture for existing longs.

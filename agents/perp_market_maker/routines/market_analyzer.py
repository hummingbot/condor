"""Hyperliquid perp regime analyzer for the Perp Market Maker.
...
# Perp take-profit fee floor: TP (= half-spread capture) must clear round-trip
# fees, so the spread can never round below this fraction.
PERP_SPREAD_FLOOR = 0.0008  # 0.08%
_HL_MAINNET = "https://api.hyperliquid.xyz"


class Config(BaseModel):
    """Analyze Hyperliquid perp regime for a coin and derive spread guidance."""
    coin: str = Field(default="ETH", description="Hyperliquid perp coin, e.g. ETH, BTC, SOL")
    venue: str = Field(default="hyperliquid", description="Perp venue (hyperliquid)")


def _hl_info():
    from hyperliquid.info import Info
    return Info(_HL_MAINNET, skip_ws=True)


def _fetch_hl_candles(info, coin: str, interval: str, lookback_ms: int) -> list:
    """Hyperliquid candles normalized to {timestamp, open, high, low, close, volume}."""
    now = int(time.time() * 1000)
    raw = info.candles_snapshot(coin, interval, now - lookback_ms, now) or []
    out = []
    for c in raw:
        out.append({
            "timestamp": c.get("t"),
...
def _fetch_hl_book(info, coin: str) -> dict:
    """HL l2Book normalized to {bids, asks} of [price, size]."""
    book = info.post("/info", {"type": "l2Book", "coin": coin})
    levels = (book or {}).get("levels") or [[], []]
    def _side(rows):
        return [[float(r["px"]), float(r["sz"])] for r in rows]
    return {"bids": _side(levels[0]), "asks": _side(levels[1])}


def _fetch_hl_funding(info, coin: str) -> str:
    """Current predicted funding rate for the coin from metaAndAssetCtxs."""
    try:
        meta, ctxs = info.meta_and_asset_ctxs()
        names = [a["name"] for a in meta["universe"]]
        i = names.index(coin)
        rate = ctxs[i].get("funding")
        return f"{float(rate) * 100:.4f}%/hr" if rate is not None else "N/A"
    except Exception:
        return "unavailable"


async def _fetch_inventory(coin: str) -> dict:
    """The agent's live NET perp position and resting maker orders for the coin,
    read with the agent's own Hyperliquid creds inside the condor process.

    This is what lets the maker run serverless: inventory comes from the venue
    here, not from the hummingbot portfolio tool. On a one-way (netted) account
    there is exactly one net position per coin — the two resting quotes push it
    up and down, and the maker re-centers off this reading."""
    ...
    try:
        ...
        return {"error": f"HL creds/client unavailable: {e}"}
    try:
        cl = HyperliquidClient(
            creds["agent_private_key"], creds["account_address"], creds["network"]
        )
        pos = await cl.position(coin)
        try:
            oo = await _a.to_thread(cl.info.open_orders, cl.account_address)
        except Exception:
            oo = []
        coin_orders = [o for o in (oo or []) if o.get("coin") == coin]
        return {
            "side": pos.side if pos else "FLAT",
            "size": float(pos.size) if pos else 0.0,
            "entry_px": float(pos.entry_px) if pos else None,
            "unrealized_pnl": float(pos.unrealized_pnl) if pos else 0.0,
            "resting_orders": len(coin_orders),
            "orders": [
                {
        ...
        return {"error": f"inventory read failed: {e}"}
...
def _spread_recommendation(regime: str) -> str:
    if regime in ("trending_up", "trending_down"):
        return "widen hard + skew toward trend (stand down if strong)"
    if regime == "volatile":
        return "widen hard (base x3-5) — fills are toxic"
    if regime == "quiet":
        return "tighten to the fee floor — harvest the chop"
...
def _regime_spread(regime: str, atr_pct_5m: float) -> dict:
    """Turn the regime into concrete spread guidance for the maker.

    Base half-spread = max(fee floor, ½ × 5m ATR%); final spread = base × the
    regime multiplier. In a strong trend, recommend standing down instead.
    """
    base = max(PERP_SPREAD_FLOOR, (atr_pct_5m / 100) * 0.5)
    mult = _REGIME_MULT.get(regime, 1.5)
    stand_down = regime in ("trending_up", "trending_down") and atr_pct_5m > 1.0
    return {
        "base_spread_pct": round(base * 100, 4),
        "multiplier": mult,
        "spread_pct": round(base * mult * 100, 4),
...
async def run(config: Config, context=None) -> str:
    ...
    pair, connector = coin, config.venue  # names reused by the report section below

    try:
        info = _hl_info()
        HOUR = 3600 * 1000
        minute_candles, candles_15m, short_candles, long_candles, ob_data_raw, funding_rate = await asyncio.gather(
            asyncio.to_thread(_fetch_hl_candles, info, coin, "1m", 24 * HOUR),
            asyncio.to_thread(_fetch_hl_candles, info, coin, "15m", 24 * HOUR),
            asyncio.to_thread(_fetch_hl_candles, info, coin, "5m", 8 * HOUR),
            asyncio.to_thread(_fetch_hl_candles, info, coin, "4h", 16 * 24 * HOUR),
            asyncio.to_thread(_fetch_hl_book, info, coin),
            asyncio.to_thread(_fetch_hl_funding, info, coin),
        )
    except Exception as e:
        return f"Error fetching Hyperliquid data for {coin}: {e}"
    ...
    funding_info = f"funding_rate: {funding_rate}"

    ob_data = _analyze_orderbook(ob_data_raw) if ob_data_raw.get("bids") else {"error": "order_book unavailable"}
    ...
    # Spread guidance — the routine's payload for the maker: regime x ATR.
    atr_5m = short.get("atr_pct", 0.0) if "error" not in short else 0.0
    spread = _regime_spread(overall_regime, atr_5m)

    # Live inventory — net position + resting quotes, so the serverless maker can
    # re-center and manage skew without the hummingbot portfolio tool (#9).
    inventory = await _fetch_inventory(coin)
    ...
    summary_parts = [
        f"**Hyperliquid Perp Regime: {coin}**",
        f"Overall Regime: **{overall_regime}**",
        f"Spread Recommendation: {_spread_recommendation(overall_regime)}",
        (f"Spread guidance: **{spread['spread_pct']}%** "
         f"(base {spread['base_spread_pct']}% x{spread['multiplier']}) "
         + ("— STAND DOWN (strong trend)" if spread["stand_down"] else "— quote both sides")),
    ]
    if inventory.get("error"):
        summary_parts.append(f"inventory: unavailable ({inventory['error']})")
    else:
        entry = inventory["entry_px"]
        summary_parts.append(
            f"inventory: net {inventory['side']} {inventory['size']}"
            + (f" @ {entry}" if entry else "")
            + f" (uPnL {inventory['unrealized_pnl']:+.4f}) | "
            f"resting_orders: {inventory['resting_orders']}"
            + (f" {inventory['orders']}" if inventory["orders"] else "")
        )
    if funding_info:
    ...
    try:
        ...
        builder = ReportBuilder(f"HL Perp Regime: {coin}")
        builder.source("routine", "market_analyzer").tags(["market-making", "regime", "hyperliquid", coin.lower()])

        builder.kpi("Overall Regime", overall_regime)
        builder.kpi("Spread", f"{spread['spread_pct']}%" + (" (STAND DOWN)" if spread["stand_down"] else ""))
        builder.kpi("Base x Mult", f"{spread['base_spread_pct']}% x{spread['multiplier']}")
